...
//            lowerThumb.setNeedsDisplay()
    ...
    var lowerAngle: Double = 270{
        didSet{
            //updateUI()
            lowerValue = lowerAngle/outputToAngleRatio
        }
    }
    ...
    var upperAngle: Double = 335{
        didSet{
            //updateUI()
            upperValue = upperAngle/outputToAngleRatio
        }
    }
    
    private var maximumAngle: Double = 0.0
    private var minimumAngle: Double = 0.0
    ...
    private var trackRadius: CGFloat = 50.0{
        didSet{
            trackLayer.setNeedsDisplay()
        }
    }
    ...
    override init(frame: CGRect) {
        ...
        titleLabel.font = UIFont(name: titleLabel.font!.fontName, size: 100.0)
    ...
    func updateUI() {
        ...
        titleLabel.frame = CGRect(x: thumbWidth, y: thumbWidth, width: (2 * trackRadius - thumbWidth), height: (2 * trackRadius - thumbWidth))
        ...
        trackLayer.setNeedsDisplay()
        trackRadius = (bounds.width - thumbWidth)/2
    ...
    func updateMinMaxValues(){
        maximumAngle = maximumValue * outputToAngleRatio
        minimumAngle = minimumValue * outputToAngleRatio
    }
    ...
    private func moveThumb(previousLocation: CGPoint, tag: Int){
        ...
        if tag==0{
            ...
            if lastAngle <= upperAngle{
                if lastAngle >= minimumAngle{
                    lowerAngle = lastAngle
                }else{
                    lastAngle = minimumAngle
                }
            }else{
            ...
            if lastAngle >= lowerAngle{
                if lastAngle <= maximumAngle{
                    upperAngle = lastAngle
                }else{
                    upperAngle = maximumAngle
                }
            }else{
